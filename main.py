...
def main() -> int:
    ...
    try:
        ...
        if args.action == 'full':
            ...
            # 执行特定步骤
            action_map = {
                'download': pipeline.deployment_manager.run_download,
                'train': pipeline.deployment_manager.run_train,
                'optimize': pipeline.deployment_manager.run_optimize,
                'deploy': pipeline.deployment_manager.run_deploy
            }
            
            success = action_map[args.action](basic_mode=args.basic)
